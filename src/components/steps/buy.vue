<template lang="pug">
  .buy-section(v-if="currentGate")
    .buy-section__caption {{ $t('buy_usdr') }}
    .buy-section__cell
      field(v-model="pay" placeholder="0" type="number" @input="payFieldMethod" :label="$t('you_pay')")
    ...
    .buy-section__cell
      field(v-model="get" @input="getFieldMethod" type="number" placeholder="0" :label="$t('you_get')")
        usdr-token
    ...
    .buy-section__button
      btn(:caption="`${$t('buying_usdr_for')} ${currentGate.name}`" :disabled="buttonValidate" @click.native="step = 'email', selectedStep = 'buy'")
</template>
...
export default {
  ...
  computed: {
    ...
    computedError() {
      ...
      if(this.get && this.get > this.currentGate.info.max_sum) compError = this.$t('max_summ_error') + " <br>" + this.currentGate.info.max_sum + " USDR";
      if(this.get && this.get > 0 && this.get < this.currentGate.info.min_sum) compError = this.$t('min_summ_error') + " <br>" + this.currentGate.info.min_sum + " USDR";
